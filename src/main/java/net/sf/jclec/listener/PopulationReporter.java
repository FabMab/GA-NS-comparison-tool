...
import m.model.ClassModelController;
import m.model.ClassModelEvaluator;
...
/**
 * *************************************************************************
 * Added JavaFX.Chart instance for plotting population results on a graph
 * 
 * 
 * @author Fabrice
 */   
 ...
 public final LineChart<Number,Number> Chart = new LineChart<>(xAxis,yAxis);
 public static XYChart.Series Cseries = new XYChart.Series();
 public static XYChart.Series Eseries = new XYChart.Series();
 ...
 //************************************************************************
	...
	protected void doIterationReport(PopulationAlgorithm algorithm, boolean force)
	{
		...
		sb.append("Fitness variance = "+ avgvar[1]+"\n");
                // @Fabrice Chart data
                IFitness bestFitness = best.getFitness();
                double fitness = ((IValueFitness)bestFitness).getValue();
                if(ClassModelEvaluator.FITNESS_MEASURE==1){
                Cseries.getData().add(new XYChart.Data(generation,fitness));
                }else{
                  Eseries.getData().add(new XYChart.Data(generation,fitness));  
                }
                //@Fabrice Added instance of ClassModelController to display class model
                if (generation== 500){
                    int[] genotype = ((IntArrayIndividual) best).getGenotype();		
		sb.append("Class model: "+ClassModelController.classModel(genotype)+"\n");
